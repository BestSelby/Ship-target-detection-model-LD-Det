...
class WTConv2d(nn.Module):
    ...
    def forward(self, x):
        device = x.device
        dtype = x.dtype  # Get the data type of the input tensor
        # Dynamically create wavelet filters based on the input tensor's dtype
        wt_filter, iwt_filter = create_wavelet_filter(
            self.wt_type, self.in_channels, self.in_channels, dtype
        )
        wt_filter = wt_filter.to(device)
        iwt_filter = iwt_filter.to(device)
        ...
        for i in range(self.wt_levels - 1, -1, -1):
            curr_x_ll = x_ll_in_levels.pop()  # 获取低频（残差）
            curr_x_h = x_h_in_levels.pop()  # 获取高频
            curr_shape = shapes_in_levels.pop()

            # **不再使用 `curr_x_ll` 进行逆变换**
            # 用全零 `torch.zeros_like(curr_x_ll)` 代替低频输入
            curr_x = torch.cat([torch.zeros_like(curr_x_ll).unsqueeze(2), curr_x_h], dim=2)
            next_x_ll = iwt_function(curr_x)  # 逆变换
            next_x_ll = next_x_ll[:, :, : curr_shape[2], : curr_shape[3]]

        # **低频部分 `x_ll` 通过残差连接**
        x_tag = next_x_ll

        x = self.base_scale(self.base_conv(x_residual))
        x = x + x_tag
